<template>
  <div class="login">
    ...
    <div class="wrapper">
      <div class="container">
        <div class="login-form" v-if="toggle==1">
          <h3>
          ...
          <div class="tips">
            <div class="sms fr">
              <span @click="toggle=2">立即注册</span> |
              <span class="forget">忘记密码</span>
            </div>
          </div>
        </div>
        <div class="login-form" v-if="toggle==2">
          <h3>
            <span class="checked">账号注册</span>
          </h3>
          <div class="input">
            <input type="text" placeholder="请输入账号" v-model="reUsername">
          </div>
          <div class="input">
            <input type="password" placeholder="请输入密码" v-model="rePassword">
          </div>
          <div class="input">
            <input type="email" placeholder="请输入邮箱" v-model="reEmail">
          </div>
          <div class="btn-box">
...
export default {
  ...
  methods: {
    login() {
      ...
      if(!username) {
        this.$message.error('用户名不能未空，请重新登录')
        return
      }
    ...
    register() {
      let { reUsername, rePassword, reEmail } = this
      if(!/^[0-9a-zA-Z_.-]+[@][0-9a-zA-Z_.-]+([.][a-zA-Z]+){1,2}$/.test(reEmail)){
